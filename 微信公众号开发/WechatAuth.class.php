...
class WechatAuth {
     ...
     * 订单号 商品名称 金额 下单时间
     */
    const TEMPLATE_ID_WAIT_PAY = '-MW990mcVi8MLGNGwGYBuaaowAtRLQHJy-XHetG4Wjc';
     ...
     * @return bool|string
     */
    protected function _rebuild_template_data($data = []){
     ...
     * @param $template_id
     * @param array $send_data['first' => 'first','remark' => 'remark', 'keys' => ['a','b']]
     * @param $url 跳转地址
     * @param $form_id 表单id或支付id
     * @param $page 点击模板卡片后的跳转页面，仅限本小程序内的页面。支持带参数,（示例index?foo=bar）。该字段不填则模板无跳转。
     * @param $emphasis_keyword 模板需要放大的关键词，不填则默认无放大
     * @return mixed|string
     */
    public function sendTemplate($touser, $template_id, $send_data, $url = '', $form_id = '', $page = '', $emphasis_keyword = ''){
        $param = [
        ...
        $data = array(
            ...
            'data' => $this->_rebuild_template_data($send_data)
        );
        ...
        $url = "{$this->apiURL}/message/wxopen/template/send";

        $result = self::http($url, $param, json_encode($data, JSON_UNESCAPED_UNICODE), 'POST');

        \Think\Log::record('模板消息数据：'. json_encode($data, JSON_UNESCAPED_UNICODE));
        ...
        $result['errmsg'] .= json_encode($data, JSON_UNESCAPED_UNICODE);
        $this->error = $result;
        return false;
    }
    ...
    public function mediaUpload($filename, $type){
        //$filename = realpath(dirname(THINK_PATH) . $filename);
        //\Think\Log::write('路径：'.realpath(dirname(THINK_PATH) . $filename));
        $filename = new \CURLFile(realpath(dirname(THINK_PATH) . $filename));
    ...
    public function messageCustomSend($openid, $content, $type = self::MSG_TYPE_TEXT){
        ...
        //根据类型附加额外数据
        $data[$type] = call_user_func(array('\Com\WechatAuth', $type), $content);

        $result = $this->api('message/custom/send', $data);
    ...
    public function createMenu($data){
    	$url  = $this->apiURL."/menu/create?access_token=".$this->accessToken;
     ...
     * @param string $type
     * @throws \Exception
     * @return mixed array(ticket,expires_in)
     */
    ...
    public function getSignPackage(){
    	if(!empty($_SESSION['ticket']) && (time()<$_SESSION['expires_in'])){
    		$jsapiTicket=$_SESSION['ticket'];
    	}else{
    		$ticket=$this->getticket();
    		$jsapiTicket =$ticket['ticket'];
    		$_SESSION['ticket']=$ticket['ticket'];
    		$_SESSION['expires_in']=time()+$ticket['expires_in'];
    	}
